...
def redirect_cas_trustee(election_uuid: str, trustee_uuid: str) -> Response:
    cas_client.service_url = (
        config["URL"]["back"] + "/" + election_uuid + "/trustee/" + trustee_uuid + "/login"
    )
    cas_login_url = cas_client.get_login_url()
    return redirect(cas_login_url)


@app.route("/<election_uuid>/trustee/<trustee_uuid>/login", methods=["GET", "POST"])
def cas_login_trustee(election_uuid: str, trustee_uuid: str) -> Response:
    """
    Make the connection and verification with the CAS service
    """
    ...
    if not ticket:
        # No ticket, the request come from end user, send to CAS login
        return redirect_cas_trustee(election_uuid, trustee_uuid)
